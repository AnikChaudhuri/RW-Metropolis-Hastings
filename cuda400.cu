...
double dch(double k11, double k12, double k13, thrust::device_vector<double>& A1, 
    ...
    thrust::device_vector<double> res_D(400);
    thrust::device_vector<double> k_1(400);
    thrust::device_vector<double> k_2(400);
    thrust::device_vector<double> k_3(400);
    thrust::device_vector<double> beta_k(400);
...
int main() {
    ...
    const int count = 400;// number of genes
    ...
    thrust::device_vector<double> d_a(400);//current alpha1
    thrust::device_vector<double> d_b(400);//current alpha2
    thrust::device_vector<double> d_c(400);//current alpha3
    ...
    thrust::device_vector<double> Acc(400);//not used anymore.

    thrust::device_vector<double> Cg(1);//sample from 1/c^2

    thrust::device_vector<double> ad1(400);//dirichlet sample, alpha1 proposal
    thrust::device_vector<double> ad2(400);//dirichlet sample, alpha2 proposal
    thrust::device_vector<double> ad3(400);//dirichlet sample, alpha3 proposal

    thrust::device_vector<double> aj1(3*400);//join all alphas
    thrust::device_vector<double> aj2(3*400);//gamma samples are stored here.
    thrust::device_vector<double> aj3(3*400);// not used anymore.

    thrust::device_vector<double> alpha1(400);//proposed alpha1 = ad1
    thrust::device_vector<double> alpha2(400);//proposed alpha2 = ad2
    thrust::device_vector<double> alpha3(400);//proposed alpha3 = ad3

    thrust::device_vector<double> Rk_p(400);//D(alphai |... )
    thrust::device_vector<double> Rk_c(400);//D(alphai |... )

    thrust::device_vector<double> Uni(3*400);//stores  1500 uniform distributed samples
    ...
    thrust::device_vector<double> d1(400);
    thrust::device_vector<double> d2(400);
    thrust::device_vector<double> d3(400);

    thrust::device_vector<double> md(400);//proposed d*proposed alpha
    thrust::device_vector<double> mc(400);//current d*current alpha

    thrust::device_vector<double> ex_alpha(400);//proposed (r-m)^2/(r^2 + m^2)
    thrust::device_vector<double> ex_c(400);//current (r-m)^2/(r^2 + m^2)
    //thrust::device_vector<double> c_alpha(400);
    thrust::device_vector<double> resp(400);//proposed eqn 16 result
    thrust::device_vector<double> resc(400);//current eqn 16 result
    ...
    double * dv_Rkp = thrust::raw_pointer_cast(Rk_p.data());// Rk eqn 400
    ...
    cudaMalloc(&d_states, sizeof(curandState_t)*3*400);
    

    dim3 gridSize(1);
    dim3 blockSize(400);
    thrust::device_vector<double> r(400);//synthetic r will be stored here.

    std::vector<double> mu(400);//m = d*alpha, mean of the normal distribution
    ...
    double c2a = (nu0 + 400.)/2.; double c_1;//c2a is the a parameter of 1/c^2
    ...
    for(int j = 0; j<num; j++){
        ...
        setup<<<3, 400>>>(d_states,j);//setting up random numbers
        ...
        thrust::copy(d_b.begin(), d_b.end(), aj1.begin()+400);
        thrust::copy(d_c.begin(), d_c.end(), aj1.begin()+(2*400));

        gen_gamma<<<3, 400>>>(dv_aj1, dv_aj2, dv_aj3, count1, d_states, U, dv_Uni);//generating gamma samples (proposals)

        //separating the generated gamma samples.
        thrust::copy_n(aj2.begin(), 400, alpha1.begin() );   //separating the alphas
        thrust::copy_n(aj2.begin()+400, 2*400, alpha2.begin() );
        thrust::copy_n(aj2.begin()+(2*400), 3*400, alpha3.begin() );
