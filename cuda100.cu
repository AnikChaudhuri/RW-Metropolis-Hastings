...
__global__ void ccopy(double *dv_Af1, double *dv_Af2, double *dv_Af3, double *dv_Ath1, double *dv_Ath2, double *dv_Ath3,
    ...
    if (idx <= count3){
        ...
        dv_Ath1[idx] = dv_Af1[num1+ (idx * 500)];
        dv_Ath2[idx] = dv_Af2[num1 + (idx * 500)];
        dv_Ath3[idx] = dv_Af3[num1 + (idx * 500)];
    }
...
//double Uk1 = 1;
double Uk1 =0.7;
double t = ((c_k1[0] + Uk1)-(c_k1[0] - Uk1)+.89)*Uni[0] + (c_k1[0] - Uk1);
...
//double Uk2 = .500;
double t2 = ((c_k2[0] + Uk1)-(c_k2[0] - Uk1)+.89)*Uni[1] + (c_k2[0] - Uk1);
...
//double Uk3 = .500;
double t3 = ((c_k3[0] + Uk1)-(c_k3[0] - Uk1)+.89)*Uni[2] + (c_k3[0] - Uk1);
...
double kp1 = ((-.5*p_k1[0]) + (-2.*p_k2[0]) + (-3.*p_k3[0]));
double kc1 = ((-.5*c_k1[0]) + (-2.*c_k2[0]) + (-3.*c_k3[0]));
...
int num1 = (.4*num) - 1;
ccopy<<<500, 1000>>>(dv_K1, dv_K2, dv_K3, dv_Kth1, dv_Kth2, dv_Kth3, count3, num1);
cudaFree(d_states);

//std::cout << "acceptance: "<< acp/(500*num) <<std::endl;
