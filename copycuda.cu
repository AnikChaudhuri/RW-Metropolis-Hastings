...
thrust::device_vector<double> d_a(500);//current
thrust::device_vector<double> d_b(500);
thrust::device_vector<double> d_c(500);

thrust::fill(d_a.begin(), d_a.end(), .6);
thrust::fill(d_b.begin(), d_b.end(), .2);
thrust::fill(d_c.begin(), d_c.end(), .1);

thrust::device_vector<double> Acc(500);

thrust::device_vector<double> Cg(1);

thrust::device_vector<double> ad1(500);//dirichlet
thrust::device_vector<double> ad2(500);
thrust::device_vector<double> ad3(500);

thrust::device_vector<double> aj1(3*500);//join all alphas
thrust::device_vector<double> aj2(3*500);//samples
thrust::device_vector<double> aj3(3*500);
/*
thrust::device_vector<double> Af1(500*num);
thrust::device_vector<double> Af2(500*num);
thrust::device_vector<double> Af3(500*num);*/

thrust::device_vector<double> alpha1(500);//proposed
thrust::device_vector<double> alpha2(500);
thrust::device_vector<double> alpha3(500);

thrust::device_vector<double> Rk_p(500);//D(alphai |... )
thrust::device_vector<double> Rk_c(500);//D(alphai |... )

thrust::device_vector<double> Uni(3*500);
...
cudaMalloc(&d_states, sizeof(curandState_t)*1500);
   

dim3 gridSize(1);
dim3 blockSize(500);
thrust::device_vector<double> r(500);
//thrust::device_vector<double> r(x2, x2 + 500);
std::vector<double> mu(500);
...
double nu0 = 1; double c20 = 0; 
double c2a = (nu0 + 500.)/2.; double c_1;
...
double a1k; long fastest=1000000;
for(int j = 0; j<num; j++){

    long start = clock();
    //std::cout << "loop " << j <<std::endl;
setup<<<2, 750>>>(d_states,j);

thrust::copy(d_a.begin(), d_a.end(), aj1.begin());  //joining all the alphas
thrust::copy(d_b.begin(), d_b.end(), aj1.begin()+500);
thrust::copy(d_c.begin(), d_c.end(), aj1.begin()+1000);

gen_gamma<<<2, 750>>>(dv_aj1, dv_aj2, dv_aj3, count1, d_states, U, dv_Uni);//generating gamma samples (proposals)

thrust::copy_n(aj2.begin(), 500, alpha1.begin() );   //separating the alphas
thrust::copy_n(aj2.begin()+500, 1000, alpha2.begin() );
thrust::copy_n(aj2.begin()+1000, 1500, alpha3.begin() );
...
alpha3 = ad3;
/*
std::cout << "current d_a" <<std::endl;
for(int i2 = 0; i2<10; i2++){
    std::cout << d_a[i2] <<std::endl;
}
std::cout << "current d_b" <<std::endl;
for(int i2 = 0; i2<10; i2++){
    std::cout << d_b[i2] <<std::endl;
}
std::cout << "current d_c" <<std::endl;
for(int i2 = 0; i2<10; i2++){
    std::cout << d_c[i2] <<std::endl;
}*/
...
/*
thrust::copy(thrust::device, d_a.begin(), d_a.end(), Af1.begin() + acc);
//acc = acc + 10;
thrust::copy(thrust::device, d_b.begin(), d_b.end(), Af2.begin() + acc);
//acc = acc + 10;
...
//double Uk1 = 1;
double Uk1 =1;
double t = ((c_k1[0] + Uk1)-(c_k1[0] - Uk1)+.45)*Uni[0] + (c_k1[0] - Uk1);
...
//double Uk2 = .10;
double t2 = ((c_k2[0] + Uk1)-(c_k2[0] - Uk1)+.45)*Uni[1] + (c_k2[0] - Uk1);
...
//double Uk3 = .10;
double t3 = ((c_k3[0] + Uk1)-(c_k3[0] - Uk1)+.45)*Uni[2] + (c_k3[0] - Uk1);
...
double kp1 = ((-1.*p_k1[0]) + (-2.*p_k2[0]) + (-3.*p_k3[0]));
double kc1 = ((-1.*c_k1[0]) + (-2.*c_k2[0]) + (-3.*c_k3[0]));
...
std::cout<<"time: "<<float(fastest)/CLOCKS_PER_SEC<<" millisecond"<<std::endl;
